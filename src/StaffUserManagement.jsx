...
export default function StaffUserManagement() {
    ...
    // ── View toggle ────────────────────────────────────────
    const [activeTab, setActiveTab] = useState('directory'); // 'directory' | 'deleted'

    // ── Search ─────────────────────────────────────────────
    const [clinicalSearchTerm, setClinicalSearchTerm] = useState('');
    ...
    const fetchDeletedStaff = async () => {
        ...
        try {
            ...
            console.error('Error fetching deleted staff');
        } finally { setDeletedLoading(false); }
    };

    useEffect(() => { fetchStaff(); }, []);
    useEffect(() => { if (activeTab === 'deleted') fetchDeletedStaff(); }, [activeTab]);
    ...
    const clinicalStaff = staffList.filter(s =>
        ['doctor', 'technician', 'receptionist', 'patient_admin'].includes(s.role)
    );

    const filteredClinicalStaff = clinicalStaff.filter(s => {
        if (!clinicalSearchTerm.trim()) return true;
        const q = clinicalSearchTerm.toLowerCase();
        return (
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editMode ? `${API}/api/admin/staff/${editId}` : `${API}/api/admin/staff`;
    ...
    const resetForm = () => {
        setFormData({ name: '', email: '', password: '', role: 'doctor', department: '', contactNumber: '', employeeId: '' });
        setEditMode(false);
        setEditId(null);
    };
    ...
    const handleToggleStatus = async (staffId, currentStatus) => {
        try {
            ...
            if (res.ok) {
                setMessage({ type: 'success', text: `Staff member ${!currentStatus ? 'enabled' : 'disabled'} successfully.` });
                fetchStaff();
            } else {
            ...
            setMessage({ type: 'error', text: 'Connection error while updating status.' });
        }
    };

    // ── Delete modal handlers ──────────────────────────────
    const openDeleteModal = (staff) => { setDeleteTarget(staff); setDeleteReason(''); };
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            if (res.ok) {
                setMessage({ type: 'success', text: `${deleteTarget.name} deleted and archived successfully.` });
                setStaffList(prev => prev.filter(s => s._id !== deleteTarget._id));
                setDeleteTarget(null);
                setDeleteReason('');
            } else {
                const errData = await res.json().catch(() => ({}));
                setMessage({ type: 'error', text: errData.message || 'Delete failed.' });
                setDeleteTarget(null);
            }
        } catch {
    ...
    const roleBadge = (role) => {
        const map = {
            ...
            patient_admin: 'bg-violet-100 text-violet-700',
        };
        ...
        <div className="flex h-screen bg-[#f8fafc] text-slate-800">
            ...
            <aside className="w-64 bg-white border-r border-gray-200 flex flex-col flex-shrink-0 fixed h-full z-20">
                <div className="p-6 border-b border-gray-100 flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-bold text-slate-800 tracking-tight">Staff<span className="text-green-600">Portal</span></h1>
                </div>
                ...
                <div className="p-4 border-t border-gray-200 bg-gray-50/50">
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex flex-col">
                            <span className="text-sm font-bold text-slate-800">{user?.name || 'Staff User'}</span>
                            <span className="text-xs text-gray-500 capitalize">{user?.role || 'Staff'}</span>
                        </div>
                    </div>
                    <button onClick={handleLogout} className="w-full text-xs font-semibold text-red-600 bg-white border border-red-100 hover:bg-red-50 py-2 rounded-lg transition-colors flex items-center justify-center gap-2">
                        Sign Out
                    </button>
                </div>
            </aside>
                ...
                <div className="bg-white border-b border-gray-200 px-8 py-5 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className={`px-4 py-2 text-sm font-semibold rounded-lg transition-all ${activeTab === 'directory' ? 'bg-green-600 text-white shadow' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                        >
                            Clinical Staff Directory
                        </button>
                        <button
                            onClick={() => setActiveTab('deleted')}
                            className={`px-4 py-2 text-sm font-semibold rounded-lg transition-all flex items-center gap-2 ${activeTab === 'deleted' ? 'bg-red-600 text-white shadow' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                        >
                    ...
                    {activeTab === 'directory' && (
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            ...
                            <div className="lg:col-span-4 flex flex-col gap-6">
                                <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 sticky top-6">
                                    <div className="flex justify-between items-center mb-6">
                                        ...
                                        {editMode && (
                                            <button
                                                onClick={() => { resetForm(); setMessage({ type: '', text: '' }); }}
                                    ...
                                    <form className="space-y-5" onSubmit={handleSubmit}>
                                        <div>
                                            ...
                                            <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="e.g. John Doe" required className="w-full px-4 py-2.5 rounded-md border border-gray-200 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all" />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-2">Email Address *</label>
                                            <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="john@hospital.com" required className="w-full px-4 py-2.5 rounded-md border border-gray-200 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all" />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-2">
                                                Password {editMode && '(Leave blank to keep current)'} *
                                            </label>
                                            <input type="password" name="password" value={formData.password} onChange={handleChange} placeholder={editMode ? 'Enter new password (optional)' : 'Enter password'} required={!editMode} className="w-full px-4 py-2.5 rounded-md border border-gray-200 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all" />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-2">Role</label>
                                            <div className="relative">
                                                <select name="role" value={formData.role} onChange={handleChange} className="w-full px-4 py-2.5 rounded-md border border-gray-200 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all appearance-none bg-white">
                                                    <option value="doctor">Doctor</option>
                                                    <option value="technician">Lab Technician</option>
                                                    <option value="receptionist">Receptionist</option>
                                                    <option value="patient_admin">Patient Management Admin</option>
                                                </select>
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold text-gray-600 uppercase tracking-wide mb-2">Department</label>
                                            <div className="relative">
                                                <select name="department" value={formData.department} onChange={handleChange} className="w-full px-4 py-2.5 rounded-md border border-gray-200 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all appearance-none bg-white">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="contactNumber" value={formData.contactNumber} onChange={handleChange} placeholder="+1 (555) 000-0000" className="w-full px-4 py-2.5 rounded-md border border-gray-200 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all" />
                                        </div>
                                        <button
                                            type="submit"
                                            disabled={loading}
                            ...
                            <div className="lg:col-span-8">
                                <div className="bg-white rounded-lg border border-gray-200 shadow-sm flex flex-col h-full max-h-[calc(100vh-160px)]">
                                    <div className="p-6 border-b border-gray-100">
                                        <div className="flex items-center justify-between mb-3">
                                            ...
                                            <span className="text-xs text-gray-400">{filteredClinicalStaff.length} result{filteredClinicalStaff.length !== 1 ? 's' : ''}</span>
                                        </div>
                                        {/* Full-width search */}
                                        <div className="relative">
                                            <input
                                                ...
                                                className="w-full pl-10 pr-4 py-2 text-sm border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all"
                                            />
                                    ...
                                    <div className="overflow-y-auto flex-1">
                                        {filteredClinicalStaff.length === 0 ? (
                                            <div className="p-10 text-center text-gray-400">
                                                <svg xmlns="http://www.w3.org/2000/svg" className="w-10 h-10 mx-auto mb-3 opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /></svg>
                                                <p className="text-sm font-medium">{clinicalSearchTerm ? `No staff matching "${clinicalSearchTerm}"` : 'No clinical staff found.'}</p>
                                            </div>
                                        ) : (
                                            <table className="w-full text-left">
                                                <thead className="sticky top-0 bg-gray-50/90 backdrop-blur-sm z-10">
                                                    <tr className="border-b border-gray-100">
                                                        ...
                                                        <th className="py-4 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[18%]">Role</th>
                                                        <th className="py-4 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[22%]">Department</th>
                                                ...
                                                <tbody className="divide-y divide-gray-50">
                                                    {filteredClinicalStaff.map(staff => (
                                                        <tr key={staff._id} className="hover:bg-gray-50/50 transition-colors">
                                                            <td className="py-4 px-6">
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <div>
                                                                        <div className="text-sm font-semibold text-slate-700">{staff.name}</div>
                                                                        <div className="text-xs text-gray-400">{staff.email}</div>
                                                                    </div>
                                                                </div>
                                                            </td>
                                                            <td className="py-4 px-4">
                                                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${roleBadge(staff.role)}`}>
                                                                    {staff.role === 'patient_admin' ? 'Patient Admin' : staff.role}
                                                                </span>
                                                            </td>
                                                            ...
                                                            <td className="py-4 px-4 text-right">
                                                                <div className="flex items-center justify-end gap-1">
                                                                    ...
                                                                    {/* Disable / Enable */}
                                                                    <button
                                                                        className={`p-1.5 rounded-lg transition-colors ${staff.isActive ? 'text-gray-400 hover:text-amber-600 hover:bg-amber-50' : 'text-gray-400 hover:text-green-600 hover:bg-green-50'}`}
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                                {[
                                    ...
                                    { label: 'Unique Reasons', value: [...new Set(deletedStaff.map(d => d.deletionReason))].length, bg: 'bg-gradient-to-br from-slate-600 to-slate-800', icon: '📝' },
                                ].map(c => (
                            ...
                            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                                <div className="p-6 border-b border-gray-100 flex items-center gap-3">
                                    <div className="w-9 h-9 rounded-xl bg-red-100 text-red-600 flex items-center justify-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" /></svg>
                                    </div>
                                    <div>
                                        <h2 className="text-base font-bold text-slate-800">Deleted Clinical Staff Archive</h2>
                                        <p className="text-sm text-gray-400">All deleted staff records are preserved here for audit purposes</p>
                                    </div>
                                </div>
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left">
                                        ...
                                        <tbody className="divide-y divide-gray-50">
                                            {deletedLoading ? (
                                                <tr><td colSpan="6" className="py-10 text-center text-gray-400">Loading deleted records...</td></tr>
                                            ) : deletedStaff.length === 0 ? (
                                                ...
                                                <tr key={i} className="hover:bg-red-50/20 transition-colors">
                                                    ...
                                                    <td className="py-4 px-4">
                                                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${roleBadge(d.role)}`}>
                                                            {d.role === 'patient_admin' ? 'Patient Admin' : d.role}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td className="py-4 px-4 text-xs text-gray-400 whitespace-nowrap">
                                                        {new Date(d.deletedAt).toLocaleString('en-IN')}
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
                        <div className="bg-gradient-to-r from-red-500 to-rose-600 px-6 py-5 flex justify-between items-center">
                            <div>
                                <h2 className="text-lg font-bold text-white">Delete Clinical Staff</h2>
                                <p className="text-red-200 text-xs">This action will archive the account permanently</p>
                            </div>
                        ...
                        <div className="p-6">
                            <div className="flex items-center gap-4 mb-5 p-4 bg-red-50 rounded-xl border border-red-100">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${roleBadge(deleteTarget.role)}`}>
                                            {deleteTarget.role === 'patient_admin' ? 'Patient Admin' : deleteTarget.role}
                                        </span>
                                        <span className="text-xs text-gray-400">{deleteTarget.department || 'No Department'}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <textarea
                                ...
                                placeholder="Please provide a clear reason why this staff account is being deleted (e.g., Resignation, Contract ended, Policy violation, Duplicate account)..."
                            ...
                            />
                            <p className="text-xs text-gray-400 mt-2">⚠️ The account will be preserved in Deleted Staff Records with this reason and your name.</p>
                        </div>
